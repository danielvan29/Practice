...
$(function() {
	...
	addBtn = $("#add");
	backBtn = $("#back");
	idPicPath = $("#a_idPicPath");
	workPicPath = $("#a_workPicPath");
	errorinfo = $("#errorinfo");
	...
	if (errorinfo.val() == null || errorinfo.val() == "") {
		idPicPath.next().html("*上传文件大小不能超过500k *上传文件类型必须是：jpg,jpeg,png,gif！");
		workPicPath.next().html("*上传文件大小不能超过500k *上传文件类型必须是：jpg,jpeg,png,gif！");
	} else {
		idPicPath.next().html(errorinfo.val());
		workPicPath.next().html(errorinfo.val());
	}
